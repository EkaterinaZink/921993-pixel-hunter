...
const answersInputsElement = screen.querySelectorAll(`input`);
answersInputsElement.forEach((item) => {
  item.addEventListener(`click`, () => {
    if (screen.querySelectorAll(`input:checked`).length > 0) {
      renderScreen(gameScreenThree);
    }
  });
});
export {screen as gameScreenTwo};
